  ...
  <div class="space-y-8">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      <NuxtLink
        ...
        :key="category.slug"
        :to="`/academy/practice/${category.slug}`"
      ...
      >
        <div class="mb-4 flex items-start justify-between">
          ...
          <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-500">
            {{ getQuestionCount(category.slug) }}問
          </span>
        </div>
...
const { categories, getPracticeQuestionsByCategory } = useAcademyMock()

function getQuestionCount(categorySlug: string) {
  return getPracticeQuestionsByCategory(categorySlug).length
}
</script>
